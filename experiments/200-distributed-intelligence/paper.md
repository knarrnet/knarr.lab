...
Wang et al. [3] proposed an architecture for Agentic Peer-to-Peer Networks that decouples agent intelligence from centralized infrastructure. Their framework identifies economic, intelligence, and trust layers as necessary components for autonomous agent cooperation. This paper asks whether those layers can be instantiated with concrete mechanisms --- specifically, whether bilateral credit, signed knowledge packs, and quality gates can support a functioning knowledge marketplace among small language model agents.
...
2. Model scaling thresholds for knowledge-augmented QA across 6 architectures on a 100-question SQuAD 2.0 subset, identifying 2.3B (extraction) and 4B (composition) as minimum sizes (Section 5.2).
...
Wang et al. [3] proposed an architecture for Agentic Peer-to-Peer Networks in which autonomous agents cooperate without centralized coordination. Their framework identifies three layers: an economic layer for incentive alignment, an intelligence layer for capability composition, and a trust layer for accountability. Our work provides implementation evidence for these layers using bilateral credit (economic), signed knowledge packs (intelligence), and quality gates (trust). We do not claim to validate their framework in full; rather, we instantiate specific mechanisms and report empirical observations on a small-scale testbed.
...
RQ4 (Trust): The quality gate caught 76% of adversarial packs with 0% false rejects. Adaptive credit tightened free-riders in a single test. A 38% adversarial miss rate remains.
...
Wang et al. [3] identified economic incentives, intelligence composition, and trust as necessary layers for agentic P2P networks. Our results, while limited in scale, suggest several implications for how these layers interact in practice.

**Knowledge as the natural unit of trade, not compute.** The zero-shot baseline (Section 5.1) shows that small models without domain knowledge perform at 12--26% on extractive QA — barely above random. With knowledge packs, the same models reach 52--90%. The value is overwhelmingly in the knowledge, not the compute. This suggests that P2P agent economies will be organized around knowledge providers (curators who invest expensive reasoning to create packs) and knowledge consumers (small models that serve from cached packs at near-zero marginal cost). The 26B curator spends 25 seconds writing a pack that a 4B agent serves thousands of times. This is a 1:N amortization pattern that bilateral credit naturally supports: the curator earns credits per pack sold, the consumer pays per query, and both benefit from the asymmetry.
...
The analogy to peer-to-peer file sharing is instructive. Napster proved that digital goods could be distributed without centralized infrastructure. BitTorrent proved that economic incentives (tit-for-tat) could sustain the network. What was missing was quality control and provenance. Knowledge packs add both: Ed25519 signatures provide provenance, quality gates provide verification, and bilateral credit provides the economic incentive to create high-quality packs rather than spam. The question is whether this composability creates emergent knowledge markets — whether nodes will spontaneously specialize, curate, and trade knowledge in ways that make the network collectively smarter. Our experiments did not test this; they tested individual transactions. But the primitives are in place.

**Adaptive credit provides reputation without consensus.** Traditional P2P reputation systems require global agreement on who is trustworthy. Bilateral credit avoids this: each node independently adjusts credit limits based on direct experience. A free-rider is bounded not by a global score but by the individual credit limits of every peer it interacts with. While our test of this mechanism is limited to a single demonstration (Phase F), the architectural implication is significant: reputation in an agent economy can be local and bilateral, avoiding the Sybil vulnerabilities that plague global reputation systems. Whether this holds under determined adversarial pressure remains untested.
